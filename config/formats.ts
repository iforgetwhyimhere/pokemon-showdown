...
export const Formats: import('../sim/dex-formats').FormatList = [
	...
	},
	{
		...
		onValidateTeam(team, format, teamHas) {
			...
			for (const set of team) {
				let species = this.dex.species.get(set.species);
				if (species.battleOnly) species = this.dex.species.get(species.battleOnly as string);
				const baseSpecies = this.toID(species.baseSpecies);
				if (heads[baseSpecies]) headCount++;
			}
